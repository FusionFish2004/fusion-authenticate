...
public abstract class SimpleCommand extends Command {
    ...
    /*
    public void inorder(SimpleCommand command, List<SimpleCommand> bufferList) {
        if (command != null) {
            inorder(command.getParent(), bufferList);
            bufferList.add(command);
            command.getSubCommands().values().forEach(command1 -> inorder(command1, bufferList));
        }
    }
